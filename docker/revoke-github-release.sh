...
docker run --rm -u "`id -u`:`id -g`" -v "$path_host:$path_volume:ro" -v "`printenv "${application_id_env}_GITHUB_TOKEN_FILE"`:/run/secrets/$application_id-github-token:ro" -e "${application_id_env}_GITHUB_TOKEN_FILE=/run/secrets/$application_id-github-token" --entrypoint "$application_id-revoke-github-release" "$image:$version" "--path=$path_volume" "$@"
