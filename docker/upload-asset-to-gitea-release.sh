...
docker run --rm -u "`id -u`:`id -g`" -v "$path_host:$path_volume:ro" $(if [ -n "${!gitea_https_certificate_file}" ]; then echo -v "`printenv "$gitea_https_certificate_file"`:/run/secrets/$application_id-gitea-https-certificate:ro" -e "NODE_EXTRA_CA_CERTS=/run/secrets/$application_id-gitea-https-certificate"; fi) -v "`printenv "${application_id_env}_GITEA_TOKEN_FILE"`:/run/secrets/$application_id-gitea-token:ro" -e "${application_id_env}_GITEA_TOKEN_FILE=/run/secrets/$application_id-gitea-token" --entrypoint "$application_id-upload-asset-to-gitea-release" "$image:$version" "--path=$path_volume" "$@"
