...
docker run --rm -u "`id -u`":"`id -g`" -v "$path_host":"$path_volume":ro -v "$PUBLISH_UTILS_GITHUB_TOKEN_FILE":/run/secrets/publish_utils_github_token:ro -e PUBLISH_UTILS_GITHUB_TOKEN_FILE=/run/secrets/publish_utils_github_token --entrypoint create-github-release "$image:$tag" "$path_volume" "$@"
